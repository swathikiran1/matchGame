...
class App extends Component {
  state = {
    ...
    activeMatchImageId: imagesList[0].id,
    isCorrectImageClicked: true,
    score: 0,
    seconds: 60,
  }
  ...
  tick = () => {
    ...
    if (seconds === 0) {
      clearInterval(this.timerId)
      this.setState({isCorrectImageClicked: false})
    }
  }
  ...
  clickThumbnailItem = thumbnailValue => {
    const {activeMatchImageId} = this.state

    const matchImageId =
      imagesList[Math.floor(Math.random() * imagesList.length)].id

    if (activeMatchImageId === thumbnailValue) {
      this.setState(prev => ({
        activeMatchImageId: matchImageId,
        isCorrectImageClicked: true,
        score: prev.score + 1,
      }))
    } else {
  ...
  clickedPlayAgainBtn = () => {
    this.setState({score: 0, seconds: 60, isCorrectImageClicked: true})
    this.timerId = setInterval(this.tick, 1000)
  }
  ...
  render() {
    const {
      activeTabId,
      activeMatchImageId,
      isCorrectImageClicked,
      score,
      seconds,
    } = this.state

    console.log(activeMatchImageId)

    const filteredImagesList = imagesList.filter(
      each => each.category === activeTabId,
    )

    const activeMatchImageUrl = imagesList.filter(
      each => each.id === activeMatchImageId,
    )[0].imageUrl
        ...
        {isCorrectImageClicked ? (
          ...
          <div className="bottom-container">
            <div className="game-over-container">
              ...
              />
              <p className="score-name1">YOUR SCORE</p>
              <p className="total-score">{score}</p>
              <button
                type="button"
                className="play-again-btn"
                onClick={this.clickedPlayAgainBtn}
              >
